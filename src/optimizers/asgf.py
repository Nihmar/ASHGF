...
class ASGF(BaseOptimizer):
    ...
    Versione corretta:
      - Stima delle costanti di Lipschitz usa l'insieme completo I (eq. 3.1).
      - Learning rate senza cap arbitrario.
      - Gauss-Hermite già corretta in numpy.
      - Costruzione della base con QR.
    """

    kind = "Adaptive Stochastic Gradient-Free (corrected)"
    ...
    def optimize(
        ...
        norm = la.norm(x)
        ASGF.data["sigma_zero"] = float(norm / 10)
        ...
        # Precompute set of index pairs I for Lipschitz estimation (eq. 3.1)
        pair_indices = [
            (a, b)
            for a in range(m)
        ...
        if debug:
            print(
                f"algorithm: asgf (corrected)  function: {function}  dimension: {dim}  initial value: {current_val}"
            )
        ...
        for i in range(1, it + 1):
            try:
                ...
                grad, lipschitz_coefficients, lr, derivatives, L_nabla = (
    ...
    def _grad_estimator(
        ...
        Also estimates local Lipschitz constants per direction (eq. 3.1) using
        the full set I of index pairs.
        """
        ...
        for j in range(dim):
            ...
            derivatives[j] = (1.0 / sigma) * np.dot(
                weights_std * nodes_std, evaluations[j]
            )
        ...
        # FIX: no artificial cap on lr
        L_nabla = (1 - ASGF.data["gamma_L"]) * lipschitz_coefficients[0] + ASGF.data[
            "gamma_L"
        ] * L_nabla
    ...
    def _subroutine(self, sigma, grad, derivatives, lipschitz_coefficients, A, B, r):
        ...
        # Ensure Lipschitz coefficients are positive
