"""ASGF-2SLS: 2SL with smoothed Lipschitz estimates.

Applies exponential smoothing to the per-direction Lipschitz constants
across iterations, reducing noise in the step-weighting signal.
"""
...
class ASGF2SLS(ASGF):
    """2SL with EMA-smoothed per-direction Lipschitz constants.
    ...
    lip_ema : float
        Exponential smoothing factor for per-direction Lipschitz.
        Default ``0.7``.
    **kwargs :
    ...
    def _compute_step(
        ...
        if confidence > 0.0:
            ...
            if self._lipschitz_smooth is None:
                self._lipschitz_smooth = lipschitz.copy()
            else:
                self._lipschitz_smooth = (
                    self._lip_ema * self._lipschitz_smooth
                    + (1.0 - self._lip_ema) * lipschitz
                )
            lipschitz = self._lipschitz_smooth
