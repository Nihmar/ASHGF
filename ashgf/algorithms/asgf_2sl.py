"""ASGF-2SL: 2S with Lipschitz-weighted per-direction acceleration.

Uses the per-direction Lipschitz estimates (already computed by ASGF)
to modulate the 2x boost: flat directions (low Lipschitz) get a larger
boost, steep directions (high Lipschitz) get a smaller one.  This uses
information that is already available at zero extra cost.
"""
...
class ASGF2SL(ASGF):
    """2S with Lipschitz-weighted per-direction acceleration.
    ...
    lip_clip : float
        Clipping factor for the Lipschitz-to-mean ratio to prevent
        extreme weights.  Default ``5.0``.
    **kwargs :
