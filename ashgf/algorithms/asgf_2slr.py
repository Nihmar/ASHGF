"""ASGF-2SLR: 2SL with square-root-compressed Lipschitz weighting.

Uses ``confidence / sqrt(ratio)`` instead of ``confidence / ratio``,
reducing the anisotropy range from ~25x to ~5x.  This makes the
per-direction acceleration milder and more robust to Lipschitz noise.
"""
...
class ASGF2SLR(ASGF):
    """2SL with sqrt-compressed Lipschitz weights.
    ...
    def _compute_step(
        ...
        if confidence > 0.0:
            ...
            if use_uniform:
                ...
                k_vec = confidence / np.sqrt(ratio)
